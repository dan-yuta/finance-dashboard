...
class FinanceApp {
    ...
    renderLoanCountdown(record) {
        ...
        const hasEnoughData = sortedRecords.length >= 2;
        container.innerHTML = `
            <div class="countdown-main">${monthsLeft > 0 ? `あと約 ${monthsLeft}ヶ月` : (hasEnoughData ? '返済ペースが利息以下です' : '2ヶ月以上のデータで完済予測を表示します')}</div>
            <div class="countdown-sub">${targetDate ? `完済予定: ${targetDate}` : ''}</div>
